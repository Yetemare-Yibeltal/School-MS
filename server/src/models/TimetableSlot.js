...
const timetableSlotSchema = new mongoose.Schema(
  {
    ...
    type: {
      type: String,
      enum: ['regular', 'lab', 'exam', 'free', 'break', 'assembly', 'sports', 'library'],
      default: 'regular',
    },
    ...
    conflictType: {
      type: String,
      enum: ['teacher_double_booking', 'room_double_booking', 'subject_overload', null],
      default: null,
    },
...
// ─── Virtuals ─────────────────────────────────
// Time range display
timetableSlotSchema.virtual('timeRange').get(function () {
...
timetableSlotSchema.virtual('label').get(function () {
  return `${this.day} Period ${this.period}: ${this.subjectName || 'Unknown'}`;
});
...
timetableSlotSchema.pre('save', function (next) {
  if (this.isModified('period') && !this.startTime) {
    const periodConfig = TIMETABLE_PERIODS.find((p) => p.period === this.period);
    if (periodConfig) {
...
timetableSlotSchema.statics.getForTimetable = function (timetableId) {
  return this.find({
...
timetableSlotSchema.statics.getDaySchedule = function (sectionId, day, academicYearId) {
  return this.find({
    section: sectionId,
    day,
...
timetableSlotSchema.statics.getTeacherSchedule = function (teacherId, academicYearId) {
  return this.find({
    teacher: teacherId,
    academicYear: academicYearId,
    isActive: true,
...
timetableSlotSchema.statics.getTeacherDaySchedule = function (teacherId, day, academicYearId) {
  return this.find({
    teacher: teacherId,
    day,
    academicYear: academicYearId,
...
timetableSlotSchema.statics.isTeacherAvailable = async function (
  teacherId,
  day,
  period,
  academicYearId,
...
timetableSlotSchema.statics.isRoomAvailable = async function (
  roomId,
  day,
  period,
  academicYearId,
...
timetableSlotSchema.statics.detectConflicts = async function (timetableId) {
  ...
  slots.forEach((slot) => {
    ...
    if (teacherSlots[teacherKey]) {
      conflicts.push({
        ...
        slotId: slot._id,
        conflictingSlotId: teacherSlots[teacherKey],
      });
  ...
  await Promise.all(
    slots.filter((s) => s.hasConflict).map((s) => s.save({ validateBeforeSave: false }))
  );
...
timetableSlotSchema.statics.assignSubstitute = async function (
  slotId,
  substituteTeacherId,
  substituteTeacherName,
  reason
...
timetableSlotSchema.statics.getSubjectSlots = function (subjectId, academicYearId) {
  return this.find({
    subject: subjectId,
    academicYear: academicYearId,
    isActive: true,
    ...
    .sort({ day: 1, period: 1 })
    .populate('section', 'fullName grade')
    .populate('teacher', 'firstName fatherName')
    .populate('room', 'name roomNumber');
};

// Count weekly periods for teacher
timetableSlotSchema.statics.countTeacherWeeklyPeriods = async function (teacherId, academicYearId) {
...
timetableSlotSchema.statics.countSubjectPeriodsForSection = async function (
  subjectId,
  sectionId,
  academicYearId
) {
...
timetableSlotSchema.statics.getTodaySchedule = function (sectionId, academicYearId) {
  const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const today = dayNames[new Date().getDay()];

  return this.getDaySchedule(sectionId, today, academicYearId);
};

// Get current period for a section
timetableSlotSchema.statics.getCurrentPeriod = async function (sectionId, academicYearId) {
  const now = new Date();
  const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
...
timetableSlotSchema.statics.bulkCreate = async function (slotsData) {
  const results = {
    created: 0,
    updated: 0,
    errors: [],
...
timetableSlotSchema.statics.clearTimetable = async function (timetableId) {
...
timetableSlotSchema.statics.getSubjectDistribution = async function (timetableId) {
  return this.aggregate([
    {
      $match: {
        timetable: new mongoose.Types.ObjectId(timetableId),
        isActive: true,
      },
    },
    {
...
timetableSlotSchema.statics.getTeacherWeeklySummary = async function (teacherId, academicYearId) {
  return this.aggregate([
    {
      $match: {
...
timetableSlotSchema.methods.conflictsWith = function (otherSlot) {
  if (this.day !== otherSlot.day || this.period !== otherSlot.period) {
    return false;
  }

  // Same teacher at same time
  if (this.teacher.toString() === otherSlot.teacher.toString()) {
    return true;
  }

  // Same room at same time
  if (this.room && otherSlot.room && this.room.toString() === otherSlot.room.toString()) {
    return true;
  }
...
timetableSlotSchema.methods.getTimeRange = function () {
  if (!this.startTime || !this.endTime) return '';
  return `${this.startTime}–${this.endTime}`;
};
